...
async def get_chain_active_agents(request: Request):
    """Return active agents from the Solana registry."""
    ...
    return {
        ...
        "combined": list(set(solana_agents)),
    }
...
class StakeRequest(BaseModel):
    ...
    amount: float
    address: str
    chain: Optional[str] = None   # "solana" | "algorand"
    txid: Optional[str] = None    # client-signed txid (Algorand flow)


@router.post("/stake")
async def stake_agent(data: StakeRequest, request: Request):
    """Stake tokens for an agent on Solana or Algorand."""
    solana   = getattr(request.app.state, "solana", None)
